...
st.caption("Upload Excel/CSV files → preview, quick analysis, choose axes for charts, and auto-generate reports with embedded charts.")
...
# Ensure session-state stores
if "axis_config" not in st.session_state:
    st.session_state["axis_config"] = {}  # key -> {"x_col":..., "y_cols":[...]}
if "chosen_for_report" not in st.session_state:
    st.session_state["chosen_for_report"] = []

# ---------------------
# Sidebar: upload/save/select files
# ---------------------
with st.sidebar:
    ...
    # Use a named session_state key for this multiselect so other code can read choices
    st.multiselect("Files to include in report", options=saved_options, default=None, key="chosen_for_report")
    st.markdown("---")
    if st.button("Open Reports Folder (in app)"):
        st.info(f"Reports are stored under: {REPORT_DIR}")

# ---------------------
# Create tabs (must come after set_page_config & header)
# ---------------------
tab_preview, tab_generate, tab_past = st.tabs(["Preview", "Generate Report", "Past Reports"])

# ---------------------
# Helper: quick analysis & auto-chart (used in preview + report fallback)
# ---------------------
...
with tab_preview:
    ...
    st.markdown("Preview session uploads or saved files — the app will run a quick analysis and draw auto-charts.")
...
# ---------------------
# Generate tab (axis chooser + generate)
# ---------------------
with tab_generate:
    st.header("Generate Report — choose axes for charts")
    st.markdown("Pick a sheet to choose X/Y axes (saved per sheet). If no config exists, the app will auto-generate charts.")
    ...
    # Build available sheets from chosen files in sidebar (session_state)
    ...
    # GENERATE: write sheets and embed charts (use axis_config if present)
    if generate:
        if not chosen_files:
